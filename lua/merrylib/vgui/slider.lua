...
function PANEL:Init()
    ...
    self.Knob:SetText( "" )
    self.Knob:SetSize( 10, 43 )
    self.Knob:NoClipping( true )
    self.Knob.Paint = function( panel, w, h ) 
        local theme = Merry.Themes[Merry.Theme]
        
        if (self.Knob.Hovered) then 
            draw.RoundedBox(10, 0, 0, w, h, theme["title_h1"])
        elseif not (self.Knob.Hovered) then
            draw.RoundedBox(10, 0, 0, w, h, theme["title_btn_hover"])
        end
    end
...
vgui.Register("MerryUI.Slider", PANEL, "Panel")