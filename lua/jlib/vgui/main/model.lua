...
--]]
    ["pm-face"] = {
        fov = 58,
        сampos = Vector(30, 0, 60),
        lookat = Vector(10, 0, 60) 
    },
    ["pm-face-little"] = {
        fov = 56,
        сampos = Vector(25, 0, 35),
        lookat = Vector(10, 0, 35)
    },
    ["pm-face-big"] = {
        fov = 86,
        сampos = Vector(25, 0, 68),
        lookat = Vector(10, 0, 68)
    },
    ["pm"] = {
        fov = 80,
        сampos = Vector(52, 0, 35),
        lookat = Vector(0, 0, 35)
    },
    ["pm-little"] = {
        fov = 65,
        сampos = Vector(52, 0, 26),
        lookat = Vector(0, 0, 26)
    },
    ["pm-big"] = {
        fov = 95,
        сampos = Vector(52, 0, 40),
        lookat = Vector(0, 0, 40)
    },    
    ["model"] = {
        fov = 70,
        сampos = Vector(52, 0, 2),
        lookat = Vector(0, 0, 2)
    },
    ["model-little"] = {
        fov = 30,
    ...
    ["model-very-little"] = {
        fov = 10,
        сampos = Vector(52, 0, 3),
        lookat = Vector(0, 0, 3)
    },
    ["model-big"] = {
        fov = 80,
        сampos = Vector(52, 0, 25),
...
function PANEL:SetValue(arg)
    ...
    if not (self.pnl_value) then
        ...
        self.pnl_value = jlib.vgui.Create("panel", self)
        self.pnl_value:SetSize(size_x*0.2, 25)
        self.pnl_value:SetPos(size_x*0.7, size_y*0.05)
        self.pnl_value:SetType("round")
        self.pnl_value.Paint = function(self, w, h)
            local parent = self:GetParent()
            if (parent.dmodel.Hovered) then
        ...
        self.txt_value = jlib.vgui.Create("label", self.pnl_value)
        self.txt_value:SetText("X" .. tostring(arg))
        self.txt_value:SetFont(jlib.vgui.GetFont(data_font, "value"))
        self.txt_value:Dock(FILL)
        self.txt_value:DockMargin(3, 3, 3, 3)
        self.txt_value:SetContentAlignment(5)
    end
end
